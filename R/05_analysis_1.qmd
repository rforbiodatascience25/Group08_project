...
```{r}
wide_annotated_sig <- annotated_sig_results |>
  mutate(
    term_clean = stringr::str_replace_all(term, "[:(), ]", "_")
  ) |>
  pivot_wider(
    id_cols = c(gene, time, condition, tissue), 
    names_from = term_clean,
    values_from = c(estimate, q.value, p.value, is_significant),
    names_glue = "{.value}_{term_clean}"
  )

...
if (!require("BiocManager", quietly = TRUE))
    install.packages("BiocManager") # has this been requested via course's slack?
...
```{r}
# Create "_processed" inside the current project folder if it doesn't exist (it wont crush if you rerun the code)
if (!dir.exists("../_raw/_processed")) dir.create("../_raw/_processed")

# Save the clean metadata inside that folder
write_csv(sample_metadata, "../_raw/_processed/sample_metadata.csv")
...
write_csv(expr_annotated, "../_raw/_processed/expression_annotated.csv")
```
...
write_csv(expr_filtered, "../_raw/_processed/expression_filtered_normalized.csv")
...
# a. Identify gene expression columns (those that are not metadata)
# We exclude the known metadata columns so everything else is a gene
metadata_cols <- c("GSM", "title", "condition", "tissue", "timepoint", "replicate")
gene_cols <- setdiff(names(expr_with_meta), metadata_cols)

# b. Define grouping columns (condition, tissue, timepoint)
group_cols <- c("condition", "tissue", "timepoint")

# c. Compute the average expression per gene for each group
...
# d. Rename the averaged columns to something like avg_AT1G01010
old_gene_names <- names(avg_values)[ -(1:length(group_cols)) ]
names(avg_values)[ -(1:length(group_cols)) ] <- paste0("avg_", old_gene_names)

# e. Merge averages back into the main dataset
...
write_csv(expr_with_meta, "../_raw/_processed/expression_with_metadata.csv")
